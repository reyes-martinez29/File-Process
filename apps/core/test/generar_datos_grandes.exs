...
Enum.each(1..10_000, fn i ->
  fecha =
    "2024-#{:rand.uniform(12) |> to_string() |> String.pad_leading(2, "0")}-#{:rand.uniform(28) |> to_string() |> String.pad_leading(2, "0")}"
  ...
  csv_content =
    csv_content <> "#{fecha},#{producto},#{categoria},#{precio},#{cantidad},#{descuento}\n"
end)
...
usuarios =
  Enum.map(1..1000, fn i ->
    %{
      ...
      "ultimo_acceso" =>
        "2024-01-#{:rand.uniform(28) |> to_string() |> String.pad_leading(2, "0")}T#{(:rand.uniform(24) - 1) |> to_string() |> String.pad_leading(2, "0")}:00:00Z"
    }
  end)

sesiones =
  Enum.flat_map(1..1000, fn user_id ->
    Enum.map(1..:rand.uniform(20), fn _ ->
      %{
        ...
        "acciones" =>
          Enum.take_random(["click", "scroll", "submit", "download", "upload"], :rand.uniform(5))
      }
    end)
  end)
...
log_content =
  Enum.map(1..50_000, fn i ->
    fecha =
      "2024-01-#{:rand.uniform(28) |> to_string() |> String.pad_leading(2, "0")} #{(:rand.uniform(24) - 1) |> to_string() |> String.pad_leading(2, "0")}:#{(:rand.uniform(60) - 1) |> to_string() |> String.pad_leading(2, "0")}:#{(:rand.uniform(60) - 1) |> to_string() |> String.pad_leading(2, "0")}"
