defmodule FProcess.CLITest do
  ...
  test "invalid options exit non-zero and show help" do
    ...
    assert exit != 0 or String.contains?(output, "Invalid options") or
  ...
  test "process directory via CLI (sequential) succeeds" do
    {output, exit} =
      System.cmd(
        "mix",
        ["run", "-e", "FProcess.CLI.main([\"data/valid\",\"--mode\",\"sequential\"])"],
        cd: @cwd,
  ...
  test "benchmark via CLI returns summary" do
    {output, exit} =
      System.cmd("mix", ["run", "-e", "FProcess.CLI.main([\"data/valid\",\"--benchmark\"])"],
        cd: @cwd,
        stderr_to_stdout: true
      )
