defmodule FProcess.Report do
  ...
  defp format_csv_file(%FileResult{status: :ok} = result) do
    ...
    best_product =
      case m[:best_selling_product] do
  ...
  defp performance_section(report) do
    ...
    benchmark_info =
      if Map.has_key?(report, :benchmark_data) && report.benchmark_data != nil do
  ...
  defp errors_section(report) do
    error_results =
      Enum.filter(report.results, fn r ->
  ...
  defp format_date_range(%{from: from_date, to: to_date})
